-- Fix v3: Correct the inheritance walk logic to properly check folder existence
-- The previous version had a logic error in checking if folder exists
...
BEGIN
    ...
    ELSE
        ...
        WHILE v_current_folder_id IS NOT NULL LOOP
            -- Check if folder exists by trying to get its parent
            ...
            -- Check if we got a result using FOUND (special PL/pgSQL variable)
            -- FOUND is automatically set to TRUE if SELECT returned a row, FALSE otherwise
            IF NOT FOUND THEN
                EXIT;
            END IF;
...
COMMENT ON FUNCTION get_effective_role IS 'Effective role on resource. Respects deny, ownership, grants, inheritance. MVP: org members get viewer on all org resources. Fixed: Properly handles file inheritance from parent folders with correct folder existence checks using FOUND diagnostic.';
