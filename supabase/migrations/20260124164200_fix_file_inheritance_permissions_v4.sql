-- Fix v4: Correct the inheritance walk - check current folder exists before getting parent
-- The previous version was getting parent_folder_id before verifying folder exists
...
BEGIN
    ...
    ELSE
        ...
        WHILE v_current_folder_id IS NOT NULL LOOP
            -- First, verify the current folder exists and get its metadata
...
COMMENT ON FUNCTION get_effective_role IS 'Effective role on resource. Respects deny, ownership, grants, inheritance. MVP: org members get viewer on all org resources. Fixed: Properly verifies folder exists before checking permissions in inheritance walk.';
