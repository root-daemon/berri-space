-- Fix: Ensure files inherit permissions correctly from parent folders
...
BEGIN
    ...
    ELSE
        ...
        WHILE v_current_folder_id IS NOT NULL LOOP
            -- Verify folder exists before checking permissions
            SELECT id INTO v_next_folder_id
            FROM folders
            WHERE id = v_current_folder_id AND deleted_at IS NULL;
            
            -- If folder doesn't exist, exit the loop
            IF v_next_folder_id IS NULL THEN
                EXIT;
            END IF;
            ...
            IF has_deny_permission(p_user_id, v_team_ids, 'folder', v_current_folder_id) THEN
                RETURN NULL;
            END IF;
            -- Reset owner_team_id before checking ancestor folder
            v_owner_team_id := NULL;
            SELECT owner_team_id INTO v_owner_team_id
            FROM folders
            WHERE id = v_current_folder_id AND deleted_at IS NULL;
            -- Only check ownership if folder was found
            ...
            IF v_role IS NOT NULL THEN
                RETURN v_role;
            END IF;
            -- Get parent folder - use temporary variables to check if folder exists
            -- Reset to NULL first to detect when SELECT finds no rows
            v_next_folder_id := NULL;
            v_next_inherit := NULL;
            SELECT parent_folder_id, inherit_permissions
            INTO v_next_folder_id, v_next_inherit
            FROM folders
            WHERE id = v_current_folder_id AND deleted_at IS NULL;
            -- If folder not found or deleted, exit the loop
            IF v_next_folder_id IS NULL THEN
                EXIT;
            END IF;
...
COMMENT ON FUNCTION get_effective_role IS 'Effective role on resource. Respects deny, ownership, grants, inheritance. MVP: org members get viewer on all org resources. Fixed: Properly handles file inheritance from parent folders.';
