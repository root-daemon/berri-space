-- Comprehensive fix: Trace through the exact issue
-- The problem is that folder role works but file role doesn't, even though inheritance should work
...
BEGIN
    ...
    -- CRITICAL: For files, v_folder_id contains the parent folder_id
    -- We need to check permissions on that folder
    IF v_inherit AND v_folder_id IS NOT NULL THEN
        -- Start with the parent folder (for files) or parent's parent (for folders)
        v_current_folder_id := v_folder_id;
        
        -- Loop through folder hierarchy
        WHILE v_current_folder_id IS NOT NULL LOOP
            -- Reset variables to detect when SELECT finds no rows
            v_current_owner_team_id := NULL;
            v_next_folder_id := NULL;
            v_next_inherit := NULL;
            v_folder_exists := FALSE;
            ...
            -- CRITICAL CHECK: Get direct grant on this folder
            -- This should find team permissions like 'viewer'
            v_role := get_direct_grant_role(p_user_id, v_team_ids, 'folder', v_current_folder_id);
            IF v_role IS NOT NULL THEN
                -- Found a permission on this folder, return it
                RETURN v_role;
            END IF;
...
COMMENT ON FUNCTION get_effective_role IS 'Effective role on resource. Respects deny, ownership, grants, inheritance. MVP: org members get viewer on all org resources. Fixed: Files properly inherit from parent folder with explicit FOUND check.';
