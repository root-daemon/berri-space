-- Final fix: Ensure file inheritance works correctly
-- The issue is that files should inherit from their parent folder
...
BEGIN
    ...
    -- For files: v_folder_id is the parent folder_id
    -- For folders: v_folder_id is the parent_folder_id
    IF NOT v_inherit THEN
        -- Inheritance disabled - skip to MVP fallback
        NULL;
    ELSIF v_folder_id IS NOT NULL THEN
...
COMMENT ON FUNCTION get_effective_role IS 'Effective role on resource. Respects deny, ownership, grants, inheritance. MVP: org members get viewer on all org resources. Fixed: Files properly inherit from parent folder.';
