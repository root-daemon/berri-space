-- Fix: Use the exact same logic as the test function which works
-- The test function finds the permission, so we'll use its exact logic
...
BEGIN
    ...
    -- Use the exact same logic as the test function which works
...
COMMENT ON FUNCTION get_effective_role IS 'Effective role on resource. Respects deny, ownership, grants, inheritance. MVP: org members get viewer on all org resources. Fixed: Uses exact same logic as test function which successfully finds inherited permissions.';
